...
interface Props {
   member: UserInfo
   isSelected: boolean
   onMemberToggle: (member: UserInfo) => void
}

function SelectMemberCard({ member, isSelected, onMemberToggle }: Props) {
   return (
      <div className='hover:bg-accent flex items-center space-x-3 rounded-lg p-2'>
         <Checkbox checked={isSelected} onCheckedChange={() => onMemberToggle(member)} />
         ...
         <div className='flex-1 cursor-pointer' onClick={() => onMemberToggle(member)}>
