...
import { getChatInfo, getUserChats } from '../services/ChatService'
...
function useChat() {
   ...
   const { data: chatInfo } = useQuery({
      queryKey: ['chatInfo', currentChatId],
      queryFn: () => getChatInfo(Number(currentChatId))
   })
         ...
         if (payload.chatId !== Number(currentChatId)) {
            queryClient.setQueryData<Chat[]>(['chats'], (oldChats) => {
               ...
               return oldChats.map((chat) =>
                  chat.chatId === payload.chatId ? { ...chat, readStatus: 'UNREAD' } : chat
               )
            })
         }
